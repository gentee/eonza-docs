...
**ref**  
Для каждой команды можно указать имя-идентификатор. Скрипт содержит стек таких имён и имеется возможность по этим именам получить информацию о том, какая часть скрипта сейчас выполняется. Например, имена-идентификаторы используются при [сохранении данных форм](autofill-forms.html) в Pro версии. Если вы показывайте одинаковые формы, но хотите чтобы использовалось разное автозаполнение, то укажите для каждой формы свой параметр *ref*.
