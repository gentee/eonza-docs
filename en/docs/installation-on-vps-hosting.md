...
Using *nginx* as a proxy server, allows you to connect Eonza to an existing domain name via https protocol. Suppose that you already have a website https://my-eonza-domain.org with [Let's Encrypt](https://letsencrypt.org) certificate and you want to open Eonza at *https://my-eonza-domain.org:5040*. To do this, add the following section to the */etc/nginx/conf.d/my-eonza-domain.org.conf* configuration file:
...
Specify the port that Eonza uses in the **proxy_pass** parameter. Save the configuration file and restart nginx (*service nginx restart*). Enter *https://my-eonza-domain.org:5040* in your browser and if everything has been configured correctly, you will see the login page of Eonza program.
